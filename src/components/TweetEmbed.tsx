...
export default function TweetEmbed({ name, handle, verified = true, content, date, likes = 842, retweets = 127 }: TweetProps) {
  return (
    <motion.div
      ...
      className="bg-[#0a1510] border border-white/10 rounded-xl p-4 my-2 max-w-full"
    >
      {/* Header */}
      <div className="flex items-center gap-2.5 mb-3">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#1F5A45] to-[#C6A85E] flex items-center justify-center text-sm font-bold text-white flex-shrink-0">
          AC
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className="text-white/90 text-sm font-bold">{name}</span>
            {verified && (
      ...
      {/* Content */}
      <div className="text-white/80 text-sm leading-relaxed whitespace-pre-line mb-3">
        {content}
      </div>

      {/* Date */}
      <div className="text-white/30 text-xs mb-3 pb-3 border-b border-white/8">
        {date}
      </div>
